...
const updateContact = async (req, res) => {
  ...
  const response = await mongodb.getDb().db().collection('contacts').replaceOne({ _id: user }, contact);
...
const deleteContact = async (req, res) => {
  const user = new ObjectId(req.params.id);
  const response = await mongodb.getDb().db().collection('contacts').deleteOne({ _id: user }, true);
